...
def mesh_index(x, y):
    """Convert coordinate (x,y) into its bit position in a packed mesh signal."""
    return x * MESH_Y + y


def set_mesh_bits(signal, active_coordinates):
    """Build and drive the complete endpoint bit mask in one write."""
    packed_value = 0
    for x, y in active_coordinates:
        packed_value |= 1 << mesh_index(x, y)
    signal.value = packed_value


def get_mesh_bit(signal, x, y):
    """Read one router endpoint's bit from a packed mesh signal."""
    return (int(signal.value) >> mesh_index(x, y)) & 1


def set_mesh_flits(signal, coordinate_flits):
    """Pack {(x,y): flit} entries and drive the complete bus in one write."""
    packed_value = 0
    for (x, y), flit in coordinate_flits.items():
        shift = mesh_index(x, y) * FLIT_WIDTH
        packed_value |= (flit & FLIT_MASK) << shift
    signal.value = packed_value


def get_mesh_flit(signal, x, y):
    """Read one endpoint's flit from its 64-bit packed section."""
    shift = mesh_index(x, y) * FLIT_WIDTH
    return (int(signal.value) >> shift) & FLIT_MASK
...
async def reset_mesh(dut):  # async allows this function to pause at await statements.
    ...
    # Clear every packed LOCAL endpoint signal before releasing reset.
    dut.local_input_flits.value = 0
    dut.local_input_valid.value = 0
    dut.local_output_ready.value = 0
...
async def test_reset_and_local_delivery(dut):
    ...
    for x in range(MESH_X):
        for y in range(MESH_Y):
            assert get_mesh_bit(dut.local_input_ready, x, y) == 1
            assert get_mesh_bit(dut.local_output_valid, x, y) == 0
    ...
    await FallingEdge(dut.clk)
    assert get_mesh_bit(dut.local_input_ready, 2, 1) == 1
    set_mesh_flits(dut.local_input_flits, {(2, 1): flit})
    set_mesh_bits(dut.local_input_valid, [(2, 1)])
    ...
    assert get_mesh_bit(dut.local_output_valid, 2, 1) == 1
    assert get_mesh_flit(dut.local_output_flits, 2, 1) == flit

    # The local endpoint accepts the flit on the next rising edge.
    set_mesh_bits(dut.local_output_ready, [(2, 1)])
    await RisingEdge(dut.clk)
    await FallingEdge(dut.clk)
    dut.local_output_ready.value = 0
    await Timer(1, unit="ns")

    assert get_mesh_bit(dut.local_output_valid, 2, 1) == 0
...
async def test_corner_to_corner_xy_routing(dut):
    ...
    await FallingEdge(dut.clk)
    assert get_mesh_bit(dut.local_input_ready, 0, 0) == 1
    set_mesh_flits(dut.local_input_flits, {(0, 0): flit})
    set_mesh_bits(dut.local_input_valid, [(0, 0)])
    ...
    for _ in range(MAX_TRAVEL_CYCLES):  # _ means the loop count is intentionally unused.
        if get_mesh_bit(dut.local_output_valid, 3, 3):
            arrived = True
    ...
    assert get_mesh_flit(dut.local_output_flits, 3, 3) == flit
    ...
    set_mesh_bits(dut.local_output_ready, [(3, 3)])
    await RisingEdge(dut.clk)
    await FallingEdge(dut.clk)
    dut.local_output_ready.value = 0
    await Timer(1, unit="ns")

    assert get_mesh_bit(dut.local_output_valid, 3, 3) == 0
...
async def test_two_flits_cross_the_mesh_in_parallel(dut):
    ...
    await FallingEdge(dut.clk)
    assert get_mesh_bit(dut.local_input_ready, 0, 0) == 1
    assert get_mesh_bit(dut.local_input_ready, 3, 3) == 1
    set_mesh_flits(
        dut.local_input_flits,
        {(0, 0): northeast_flit, (3, 3): southwest_flit},
    )
    set_mesh_bits(dut.local_input_valid, [(0, 0), (3, 3)])
    ...
    for _ in range(MAX_TRAVEL_CYCLES):
        northeast_arrived = get_mesh_bit(dut.local_output_valid, 3, 3)
        southwest_arrived = get_mesh_bit(dut.local_output_valid, 0, 0)
    ...
    assert get_mesh_flit(dut.local_output_flits, 3, 3) == northeast_flit
    assert get_mesh_flit(dut.local_output_flits, 0, 0) == southwest_flit

    # Accept both destination outputs on the same rising edge.
    set_mesh_bits(dut.local_output_ready, [(3, 3), (0, 0)])
    await RisingEdge(dut.clk)
    await FallingEdge(dut.clk)
    dut.local_output_ready.value = 0
    await Timer(1, unit="ns")
    ...
    assert get_mesh_bit(dut.local_output_valid, 3, 3) == 0
    assert get_mesh_bit(dut.local_output_valid, 0, 0) == 0
...
async def test_destination_backpressure_holds_flit(dut):
    ...
    arrived = False
    for _ in range(MAX_TRAVEL_CYCLES):
        if get_mesh_bit(dut.local_output_valid, 3, 0):
            arrived = True
            break
    ...
    assert get_mesh_flit(dut.local_output_flits, 3, 0) == flit

    # output_ready stays 0 for three cycles, so valid and data must remain stable.
    for _ in range(3):
        assert get_mesh_bit(dut.local_output_valid, 3, 0) == 1
        assert get_mesh_flit(dut.local_output_flits, 3, 0) == flit
        await RisingEdge(dut.clk)
        await FallingEdge(dut.clk)
        await Timer(1, unit="ns")
    ...
    set_mesh_bits(dut.local_output_ready, [(3, 0)])
    await RisingEdge(dut.clk)
    await FallingEdge(dut.clk)
    dut.local_output_ready.value = 0
    await Timer(1, unit="ns")

    assert get_mesh_bit(dut.local_output_valid, 3, 0) == 0
...
async def test_analysis_traffic_and_performance_numbers(dut):
    ...
    dut.local_output_ready.value = (1 << (MESH_X * MESH_Y)) - 1
    ...
    for cycle in range(1, measurement_cycles + 1):
        ...
        for source, pending_packet_ids in pending_by_source.items():
            if pending_packet_ids:
                ...
                active_source_flits[source] = packet_details[packet_id]["flit"]

        # Write each packed bus once so simultaneous sources cannot overwrite
        # one another inside the same simulator timestep.
        set_mesh_flits(dut.local_input_flits, active_source_flits)
        set_mesh_bits(dut.local_input_valid, active_source_flits.keys())
        ...
        for source, pending_packet_ids in pending_by_source.items():
            if pending_packet_ids:
                ...
                if (
                    get_mesh_bit(dut.local_input_valid, source_x, source_y)
                    and get_mesh_bit(dut.local_input_ready, source_x, source_y)
                ):
        ...
        for x in range(MESH_X):
            for y in range(MESH_Y):
                output_valid = get_mesh_bit(dut.local_output_valid, x, y)
                output_ready = get_mesh_bit(dut.local_output_ready, x, y)

                if output_valid and output_ready:
                    received_flit = get_mesh_flit(dut.local_output_flits, x, y)
