...
async def drive_and_check(
    ...
    # Pack the five request vectors into one 25-bit signal.
    packed_requests = 0
    for output_port in range(NUM_PORTS):
        packed_requests |= request_vectors[output_port] << (output_port * NUM_PORTS)
    dut.requests.value = packed_requests

    # Check every grant before the next edge accepts or holds it.
    await Timer(1, unit="ns")

    packed_grants = int(dut.grants.value)
    for output_port in range(NUM_PORTS):
        actual = (packed_grants >> (output_port * NUM_PORTS)) & 0b11111
