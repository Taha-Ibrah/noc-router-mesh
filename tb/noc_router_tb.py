...
async def reset_router(dut):  # async allows this function to pause at await statements.
    ...
    for port in range(NUM_PORTS):  # range(5) produces port indexes 0 through 4.
        dut.input_flits[port].value = 0
...
async def test_reset_clears_router(dut):
    ...
    dut.input_flits[LOCAL_PORT].value = flit
    ...
    assert int(dut.output_flits[EAST_PORT].value) == flit
...
async def test_all_routes_and_crossbar_connections(dut):
    ...
    await FallingEdge(dut.clk)
    assert int(dut.input_ready.value) == ALL_PORTS
    dut.input_flits[LOCAL_PORT].value = local_input_flit
    dut.input_flits[NORTH_PORT].value = north_input_flit
    dut.input_flits[EAST_PORT].value = east_input_flit
    dut.input_flits[SOUTH_PORT].value = south_input_flit
    dut.input_flits[WEST_PORT].value = west_input_flit
    ...
    assert int(dut.output_flits[EAST_PORT].value) == local_input_flit
    assert int(dut.output_flits[SOUTH_PORT].value) == north_input_flit
    assert int(dut.output_flits[WEST_PORT].value) == east_input_flit
    assert int(dut.output_flits[LOCAL_PORT].value) == south_input_flit
    assert int(dut.output_flits[NORTH_PORT].value) == west_input_flit
...
async def test_east_output_round_robin_order(dut):
    ...
    await FallingEdge(dut.clk)
    for input_port in range(NUM_PORTS):
        dut.input_flits[input_port].value = contenders[input_port]
    dut.input_valid.value = ALL_PORTS
    ...
    for input_port in range(NUM_PORTS):
        assert int(dut.output_valid.value) == (1 << EAST_PORT)
        assert int(dut.output_flits[EAST_PORT].value) == contenders[input_port]
...
async def test_stalled_grant_holds_the_same_flit(dut):
    ...
    await FallingEdge(dut.clk)
    dut.input_flits[EAST_PORT].value = first_flit
    ...
    assert int(dut.output_valid.value) == (1 << EAST_PORT)
    assert int(dut.output_flits[EAST_PORT].value) == first_flit
    ...
    dut.input_flits[LOCAL_PORT].value = later_flit
    ...
    assert int(dut.output_valid.value) == (1 << EAST_PORT)
    assert int(dut.output_flits[EAST_PORT].value) == later_flit
...
async def test_fifo_backpressure_and_order(dut):
    ...
    assert int(dut.output_flits[NORTH_PORT].value) == queued_flits[0]
    ...
    await FallingEdge(dut.clk)
    dut.input_flits[LOCAL_PORT].value = rejected_flit
    dut.input_valid.value = 1 << LOCAL_PORT
    await RisingEdge(dut.clk)
    await FallingEdge(dut.clk)
    dut.input_valid.value = 0
    ...
    for expected_flit in queued_flits:
        assert int(dut.output_valid.value) == (1 << NORTH_PORT)
        assert int(dut.output_flits[NORTH_PORT].value) == expected_flit
        await RisingEdge(dut.clk)
        await FallingEdge(dut.clk)
        await Timer(1, unit="ns")
